...
def generar_grafica_dimensiones(diag, titulo: str = "Resultado por dimensión (IDI-MIPG)") -> io.BytesIO:
    ...
    dimensiones = sorted(dimensiones, key=lambda r: (valor_protagonista_dimension(r) if valor_protagonista_dimension(r) is not None else 0))

    etiquetas = [f"{r.codigo} {r.nombre}" for r in dimensiones]
    valores = [valor_protagonista_dimension(r) or 0 for r in dimensiones]
