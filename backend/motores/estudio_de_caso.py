...
def _construir_escenarios_prospectivos(nombre_entidad, diag, resultado_360=None):
    ...
    escenarios = [
        ...
        (
            ...
            "detectadas —con los elementos mínimos de un plan de mejoramiento (hallazgo, causa "
...
def generar_estudio_de_caso_docx(
    ...
    tipo_regimen_especial=None, recomendaciones_completas=None,
):
    ...
    # 6.1 Plan de Mejoramiento (elementos mínimos exigidos, sin formato único obligatorio)
    doc.add_heading("6.1 Plan de Mejoramiento — estructura con los elementos mínimos exigidos", level=2)
    doc.add_paragraph(
        "Función Pública no publica un único formato de Excel/Word idéntico y obligatorio "
        "para todas las entidades del país; cada entidad adapta su propia plantilla. El "
        "contenido mínimo sí es consistente en toda la administración pública colombiana "
        "(Ley 87 de 1993, Decreto 1499 de 2017, Circular 100-003 de 2010 del DAFP, Norma "
        "15.1 NOGAI™): hallazgo, causa, acción de mejora, responsable y fechas. Se "
        "consolidan a continuación TODAS las brechas detectadas — no una selección — "
        "como exige un ejercicio de auditoría real."
    )
    ...
    doc.add_paragraph(
        "Con base en los elementos mínimos de seguimiento (Estado, fecha de seguimiento, "
        "observación) que toda Oficina de Control Interno debe registrar, se sugieren los "
    ...
    from backend.base_conocimiento.contenido_riesgo_auditoria import agregar_capitulo_riesgos_y_auditoria_docx, agregar_capitulo_plan_mejoramiento_formal_docx
    agregar_capitulo_riesgos_y_auditoria_docx(doc, diag, cruce_recomendaciones, nombre_entidad,
                                               numero_seccion_riesgos=8, numero_seccion_auditoria=9,
                                               numero_seccion_ptep=10, incluir_roles_oci=False,
                                               recomendaciones_completas=recomendaciones_completas)
    agregar_capitulo_plan_mejoramiento_formal_docx(doc, diag, cruce_recomendaciones, nombre_entidad, numero_seccion=11)
...
def generar_estudio_de_caso_pdf(
    ...
    # 6.1 Plan de Mejoramiento (elementos mínimos exigidos, sin formato único obligatorio)
    elementos.append(Paragraph("6.1 Plan de Mejoramiento — estructura con los elementos mínimos exigidos", estilo_h2))
    elementos.append(Paragraph(
        "Función Pública no publica un único formato de Excel/Word idéntico y obligatorio "
        "para todas las entidades del país; cada entidad adapta su propia plantilla. El "
        "contenido mínimo sí es consistente en toda la administración pública colombiana "
        "(Ley 87 de 1993, Decreto 1499 de 2017, Circular 100-003 de 2010 del DAFP), "
        "consolidando TODAS las brechas detectadas — no una selección.",
        estilo_normal,
    ))
    elementos.append(Spacer(1, 6))
